...
def back_to_black(speed, port=LINE_PORT_RIGHT, thresh=THRESH): 
	KIPR.mav(LMOTOR, -speed)
  	KIPR.mav(RMOTOR, -speed)
	while KIPR.analog(port) < thresh:
		pass
...
def line_follow_dist_right(speed, dist):
	print("Line following!!")
	KIPR.cmpc(LMOTOR)
	KIPR.cmpc(RMOTOR)
	while KIPR.gmpc(LMOTOR) <= dist:
...
def line_follow__back_dist(speed, dist):
	print("Line following!!")
	KIPR.cmpc(LMOTOR)
	KIPR.cmpc(RMOTOR)
	while KIPR.gmpc(LMOTOR) <= dist:
